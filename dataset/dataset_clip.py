...
class Dataset(data.Dataset):
    ...
    # Constants.
    EMPTY_IMAGE = torch.zeros(3, DatasetConfig.clip_image_size,
                              DatasetConfig.clip_image_size)
    EMPTY_PRODUCT_TEXT = ''

    def __init__(self, task, image_paths, dialogs, tokenizer = None):
        self.task = task
    ...
    def __getitem__(self, index: int):
        ...
        pos_products, pos_sample_id = self._get_images_product_texts(utter.pos_images, utter.pos_images_num, utter.pos_images_weights, DatasetConfig.max_pos_num)
        neg_products, neg_sample_id = self._get_images_product_texts(utter.neg_images, utter.neg_images_num, utter.neg_images_weights, DatasetConfig.max_neg_num)
    ...
    def _get_context_dialog(self, dialog):
        ...
        for utter_id, utter in enumerate(dialog[:-1]):
            ...
            if self.tokenizer==None:
                text_speaker = [0] * DatasetConfig.dialog_text_max_len
            else:
        ...
        if self.tokenizer==None:
            text_type.append([0] * DatasetConfig.dialog_text_max_len)
        else:
            ...
            for img_id in utter.pos_images:
                ...
                if path and isfile(path):
                    try:
                        ...
                        if self.tokenizer==None:
                            image = DatasetConfig.clip_transform(raw_image)
                        else:
    ...
    def _get_images_product_texts(self, image_ids, image_num, image_weight, max_image_num):
        ...
        texts = product_texts
        product_texts = {}
        product_texts['input_ids'] = clip.tokenize(texts, context_length = DatasetConfig.clip_max_len, truncate = True)
        product_texts['attention_mask'] = product_texts['input_ids'].gt(0).int()

        if self.tokenizer==None:
            product_texts['text_type'] = torch.tensor([[0] * DatasetConfig.product_text_max_len] * len(images))
        else:
        ...
        return (valid_image_num, images, product_texts), img_sample_id
